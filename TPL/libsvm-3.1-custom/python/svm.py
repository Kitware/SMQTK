...
def load_svm_lib():
    ...
    while root_dir is not None:
        for libname in libnames:
            try:
                return CDLL(path.join(root_dir, libdir, libname))
            except:
                pass
            try:
                return CDLL(path.join(root_dir, 'build', libdir, libname))
            except:
                pass
    ...
    # If we didn't find the library so far, try loading without path
    for libname in libnames:
        try:
            return CDLL(libname)
        except:

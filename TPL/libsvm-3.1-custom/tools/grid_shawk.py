...
c_begin, c_end, c_step = -1,  9, 2
g_begin, g_end, g_step =  -1, -11, -2
...
#          (svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        cmdline = '{0} -c {1} -g {2} -v {3}  -w1 4 -w-1 1 {4} {5}'.format \
          (svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        print( cmdline )
...
#          (self.host,self.cwd, \
#           svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        cmdline = 'ssh -x {0} "cd {1}; {2} -c {3} -g {4} -v {5}  -w1 4 -w-1 1 {6} {7}"'.format \
          (self.host,self.cwd, \
           svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        print( cmdline )
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout

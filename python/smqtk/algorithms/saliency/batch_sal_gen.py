...
class Batch_ImageSaliencyMapGenerator(ImageSaliencyMapGenerator):
    """
    ...
    def generate(self, base_image_list, augmenter, descriptor_generator,
                 blackbox):
        """
        ...
        for base_image in base_image_list:
            org_hws.append(base_image.size)
            org_imgs.append(copy.deepcopy(base_image))
            base_image_PILs.append(base_image.resize((224,224), PIL.Image.BILINEAR))
            augs_, masks = augmenter.augment(np.array(base_image_PILs[-1]))
            augs.append(augs_)
        ...
        scalar_vec = scalar_vec.reshape(len(org_imgs), -1)
        
        sal_map_rets = []
        for i in range(len(org_imgs)):
            final_sal_map = weighted_avg(scalar_vec[i], np.asarray(masks))
            final_sal_map_PIL = final_sal_map.resize((org_hws[i]), PIL.Image.BILINEAR)
            sal_map_rets.append(overlay_saliency_map(np.array(final_sal_map_PIL), np.array(org_imgs[i])))
        return sal_map_rets

IMG_SALIENCY_GENERATOR_CLASS=Batch_ImageSaliencyMapGenerator

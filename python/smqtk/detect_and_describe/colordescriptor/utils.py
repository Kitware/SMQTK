...
def generate_descriptors(cd_exe, img_filepath, descriptor_type,
                         feat_matrix_path, descr_matrix_path,
    ...
    :param feat_matrix_path: Path to where the computed feature matrix
        for the given file should be saved. This will be saved as a numpy
        binary file (.npy).
    :type det_matrix_path: str
    ...
    try:
        ...
        features, descriptors = DescriptorIO.readDescriptors(tmp_path)
    except IOError, ex:
    ...
    # limit.
    if limit_descriptors and features.shape[0] > limit_descriptors:
        idxs = numpy.random.permutation(numpy.arange(features.shape[0]))[:limit_descriptors]
        idxs = sorted(idxs)
        features = features[idxs, :]
        descriptors = descriptors[idxs, :]

    numpy.save(feat_matrix_path, features)
    numpy.save(descr_matrix_path, descriptors)
    return features.shape, descriptors.shape
